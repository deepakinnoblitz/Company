...
export function AccountImportDialog({ open, onClose, onRefresh }: Props) {
    ...
    useEffect(() => {
        if (open) {
            getDocFields('Accounts').then(setDocFields).catch(console.error);
        }
    }, [open]);
    ...
    const renderUpload = (
        <Box sx={{ py: 4, display: 'flex', flexFlow: 'column', alignItems: 'center' }}>
            ...
            <Button
                ...
                onClick={() => window.open(getTemplateUrl('Accounts'), '_blank')}
                sx={{ mt: 2 }}
            >
                ...
                <Table stickyHeader size="small">
                    <TableHead>
                        <TableRow>
                            ...
                            <TableCell>Account Field</TableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {previewData?.columns
