...
export function CallsReportView() {
    ...
    const handleExport = async (selectedFields: string[], format: 'excel' | 'csv') => {
        ...
        try {
            ...
            const filters: any[] = [['HD Call Log', 'name', 'in', idsToExport]];
                            ...
                            <Table size="medium" stickyHeader>
                                <TableHead>
                                    <TableRow sx={{ bgcolor: '#f4f6f8' }}>
                                        ...
                                        <TableCell sx={{ fontWeight: 700, color: 'text.secondary' }}>End Time</TableCell>
                                        ...
                                        .map((row, index) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <TableCell>{row.call_start_time ? new Date(row.call_start_time).toLocaleString() : '-'}</TableCell>
                                                    <TableCell>{row.call_end_time ? new Date(row.call_end_time).toLocaleString() : '-'}</TableCell>
                                                    <TableCell>{row.owner_name}</TableCell>
                                                    <TableCell>{row.enable_reminder ? 'Yes' : 'No'}</TableCell>
                                                    <TableCell
                                                        align="center"
                                                        sx={{
                                                            position: 'sticky',
...
function SummaryCard({ title, value, color }: { title: string; value: number; color: string }) {
    const getIcon = () => {
        switch (title) {
            case 'Total Calls': return 'solar:phone-calling-rounded-bold-duotone';
            case 'Scheduled Calls': return 'solar:calendar-mark-bold-duotone';
            case 'Completed Calls': return 'solar:check-circle-bold-duotone';
