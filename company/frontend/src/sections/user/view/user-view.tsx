...
export function UserView() {
  ...
  const handleCreate = async () => {
    try {
      ...
      console.error(err);
      setSnackbar({ open: true, message: currentLeadId ? 'Failed to update lead' : 'Failed to create lead', severity: 'error' });
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <>
      ...
      <Dialog open={openCreate} onClose={handleCloseCreate} fullWidth maxWidth="md">
        ...
        <Box sx={{ px: 2, borderBottom: 1, borderColor: 'divider' }}>
          ...
          >
            ...
            <Tab label="Pipeline" value="pipeline" disabled={!currentLeadId} />
            <Tab label="Followups" value="followups" disabled={!currentLeadId} />
            <Tab label="Convert Lead" value="convert" disabled={!currentLeadId || viewOnly} />
          </Tabs>
        </Box>
          ...
          {currentTab === 'general' && (
            ...
            >
              ...
              <TextField
                ...
                label="Leads Type"
                value={leadsType}
                onChange={(e) => setLeadsType(e.target.value as any)}
                required
                disabled={viewOnly}
      ...
      <Dialog open={!!pendingWorkflowChange} onClose={() => setPendingWorkflowChange(null)}>
        <DialogTitle>Confirm Workflow Change</DialogTitle>
        <DialogContent>
          Are you sure you want to change the workflow status to &quot;{pendingWorkflowChange?.action}&quot;?
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setPendingWorkflowChange(null)}>Cancel</Button>
          <Button
            onClick={async () => {
